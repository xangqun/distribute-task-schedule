        ...
        registryThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (!toStop) {
                    try {
                        ...
                        for (FactoryBean adminBiz: XxlJobExecutor.getAdminBizList()) {
                            try {
                                ReturnT<String> registryResult = ((AdminBiz)adminBiz.getObject()).registry(registryParam);
                ...
                try {
                    ...
                    for (FactoryBean adminBiz: XxlJobExecutor.getAdminBizList()) {
                        try {
                            ReturnT<String> registryResult =  ((AdminBiz)adminBiz.getObject()).registryRemove(registryParam);
                            if (registryResult!=null && ReturnT.SUCCESS_CODE == registryResult.getCode()) {
                                registryResult = ReturnT.SUCCESS;
