...
public class XxlJobServiceImpl implements XxlJobService {
	...
	public ReturnT<String> update(XxlJobInfo jobInfo) {
		...
		exists_jobInfo.setExecuteTimeout(jobInfo.getExecuteTimeout());
...
//		}
		...
		if (CollectionUtils.isNotEmpty(triggerCountMapAll)) {
			for (Map<String, Object> item: triggerCountMapAll) {
				...
				int triggerDayCount =item.get("triggerdaycount")==null?0: Integer.valueOf(String.valueOf(item.get("triggerdaycount")));
				int triggerDayCountRunning =item.get("triggerdaycountrunning")==null?0: Integer.valueOf(String.valueOf(item.get("triggerdaycountrunning")));
				int triggerDayCountSuc =item.get("triggerdaycountsuc")==null?0: Integer.valueOf(String.valueOf(item.get("triggerdaycountsuc")));
...
//		LocalCacheUtil.set(cacheKey, result, 60*1000);     // cache 60s
  ...
  @Override
  public ReturnT<Integer> addAndReturnJobId(XxlJobInfo jobInfo) {
 // valid
    XxlJobGroup group = xxlJobGroupDao.load(jobInfo.getJobGroup());
    if (group == null) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_please_choose")+I18nUtil.getString("jobinfo_field_jobgroup")), -1);
    }
    if (!CronExpression.isValidExpression(jobInfo.getJobCron())) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, I18nUtil.getString("jobinfo_field_cron_unvalid"), -1);
    }
    if (StringUtils.isBlank(jobInfo.getJobDesc())) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_please_input")+I18nUtil.getString("jobinfo_field_jobdesc")), -1);
    }
    if (StringUtils.isBlank(jobInfo.getAuthor())) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_please_input")+I18nUtil.getString("jobinfo_field_author")), -1);
    }
    if (ExecutorRouteStrategyEnum.match(jobInfo.getExecutorRouteStrategy(), null) == null) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_executorRouteStrategy")+I18nUtil.getString("system_unvalid")), -1);
    }
    if (ExecutorBlockStrategyEnum.match(jobInfo.getExecutorBlockStrategy(), null) == null) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_executorBlockStrategy")+I18nUtil.getString("system_unvalid")), -1);
    }
    if (ExecutorFailStrategyEnum.match(jobInfo.getExecutorFailStrategy(), null) == null) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_executorFailStrategy")+I18nUtil.getString("system_unvalid")), -1);
    }
    if (GlueTypeEnum.match(jobInfo.getGlueType()) == null) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_gluetype")+I18nUtil.getString("system_unvalid")), -1);
    }
    if (GlueTypeEnum.BEAN==GlueTypeEnum.match(jobInfo.getGlueType()) && StringUtils.isBlank(jobInfo.getExecutorHandler())) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_please_input")+"JobHandler"), -1);
    }
    ...
    if (StringUtils.isNotBlank(jobInfo.getChildJobId())) {
      ...
      for (String childJobIdItem: childJobIds) {
        if (StringUtils.isNotBlank(childJobIdItem) && StringUtils.isNumeric(childJobIdItem)) {
          XxlJobInfo childJobInfo = xxlJobInfoDao.loadById(Integer.valueOf(childJobIdItem));
          if (childJobInfo==null) {
            return new ReturnT<Integer>(ReturnT.FAIL_CODE,
                MessageFormat.format((I18nUtil.getString("jobinfo_field_childJobId")+"({0})"+I18nUtil.getString("system_not_found")), childJobIdItem), -1);
          }
        } else {
          return new ReturnT<Integer>(ReturnT.FAIL_CODE,
              MessageFormat.format((I18nUtil.getString("jobinfo_field_childJobId")+"({0})"+I18nUtil.getString("system_unvalid")), childJobIdItem), -1);
        }
      }
      jobInfo.setChildJobId(StringUtils.join(childJobIds, ","));
    }
    ...
    xxlJobInfoDao.save(jobInfo);
    if (jobInfo.getId() < 1) {
      return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_add")+I18nUtil.getString("system_fail")), -1);
    }
    ...
    // add in quartz
        ...
        try {
            XxlJobDynamicScheduler.addJob(qz_name, qz_group, jobInfo.getJobCron());
            //XxlJobDynamicScheduler.pauseJob(qz_name, qz_group);
            return new ReturnT<Integer>(jobInfo.getId());
        } catch (SchedulerException e) {
            logger.error(e.getMessage(), e);
            try {
                xxlJobInfoDao.delete(jobInfo.getId());
            ...
            return new ReturnT<Integer>(ReturnT.FAIL_CODE, (I18nUtil.getString("jobinfo_field_add")+I18nUtil.getString("system_fail"))+":" + e.getMessage(), -1);
        }
  }
